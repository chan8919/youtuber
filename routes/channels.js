...
router
    .route('/:id')
    .get(param('id').notEmpty().withMessage('채널 id가 잘못 들어왔습니다'), async (req, res) => {
        //개별 조회
        const validatorErr = validationResult(req);
        if (!validatorErr.isEmpty()) {
            console.log(validatorErr.array());
    ...
    .put([param('id').notEmpty().withMessage('id가필요합니다')], async (req, res) => {
        //채널 수정 
        const validatorErr = validationResult(req);
        if (!validatorErr.isEmpty()) {
            console.log(validatorErr.array());
    ...
    .delete(param('id').notEmpty().withMessage('삭제할 채널의 id가 필요합니다다'), async (req, res) => {
        // 채널 개별 삭제
        const channelId = parseInt(req.params.id);
        const validatorErr = validationResult(req);
        if (!validatorErr.isEmpty()) {
...
router
    .route('/user')
    .post(body('email').notEmpty().withMessage('이메일이 누락되었습니다다'), async (req, res) => {
        const validatorErr = validationResult(req);
        if (!validatorErr.isEmpty()) {
            res.status(400).json({ "message": validatorErr });
            return;
        }
